...
export default function Folgas() {
  ...
  useEffect(() => { api.get('/folgas').then(r=>setLista(r.data)); api.get('/motoristas').then(r=>setMotoristas(r.data)); }, []);

  async function salvar(e) {
    e.preventDefault();
    try { await api.post('/folgas', form); toast.success('Folga registrada'); setShowForm(false); setForm({ motoristaId:'', periodo:'', quantidadeDias:'' }); api.get('/folgas').then(r=>setLista(r.data)); } catch {}
  }
    ...
    <div>
        ...
        <div style={{ background:'#fff',borderRadius:12,padding:20,marginBottom:16,border:'1px solid #e5e7eb' }}>
          <form onSubmit={salvar}>
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12 }}>
              ...
              <div><label style={{ display:'block',fontSize:11,fontWeight:500,color:'#6b7280',marginBottom:4 }}>PERÍODO</label><input type="date" value={form.periodo} onChange={e=>setForm(f=>({...f,periodo:e.target.value}))} required style={{ width:'100%',padding:'8px 10px',border:'1px solid #d1d5db',borderRadius:8,fontSize:13,boxSizing:'border-box' }}/></div>
            ...
            {form.quantidadeDias && <div style={{ margin:'12px 0',padding:'10px 14px',background:'#f5f3ff',borderRadius:8,fontSize:14,color:'#7c3aed',fontWeight:600 }}>{form.quantidadeDias} dias × R$ 150,00 = {valor(form.quantidadeDias)}</div>}
      ...
      <div style={{ background:'#fff',borderRadius:12,border:'1px solid #e5e7eb',overflow:'hidden' }}>
        <table style={{ width:'100%',borderCollapse:'collapse',fontSize:13 }}>
          <thead><tr style={{ background:'#f9fafb' }}>{['Motorista','Período','Dias','Valor','→ Solicitação',...(isAdmin?['Alteração']:[])].map(h=><th key={h} style={{ padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:600,color:'#6b7280',textTransform:'uppercase',borderBottom:'1px solid #e5e7eb' }}>{h}</th>)}</tr></thead>
          <tbody>
            {lista.map(f=>(
              <tr key={f.id} style={{ borderBottom:'1px solid #f3f4f6' }}>
                ...
                <td style={{ padding:'10px 14px',color:'#7c3aed',fontWeight:500 }}>{valor(f.quantidadeDias)}</td>
                <td style={{ padding:'10px 14px' }}><input type="checkbox" checked={f.enviado} onChange={e=>api.patch(`/folgas/${f.id}/enviado`,{enviado:e.target.checked}).then(()=>api.get('/folgas').then(r=>setLista(r.data)))} style={{ accentColor:'#7c3aed' }}/></td>
