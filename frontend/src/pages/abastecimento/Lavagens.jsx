...
const vazioForm = { placa:'', frota:'buzin', tipoCaminhaoId:'', fornecedorId:'', valor:'', data:'', observacao:'' };
...
export default function Lavagens() {
  ...
  const [aba,         setAba]         = useState('historico'); // 'historico' | 'resumo'
  ...
  async function carregarLavagens() {
    ...
    try {
      ...
      setLavagens(rl.data);
      setResumo(rr.data);
    } catch { toast.error('Erro ao carregar lavagens'); }
    finally { setLoading(false); }
  }

  // Preço automático ao selecionar fornecedor + tipo
  useEffect(() => {
    if (!form.fornecedorId || !form.tipoCaminhaoId) return;
    const forn = fornecedores.find(f => f.id === form.fornecedorId);
    if (!forn) return;
    const preco = forn.precos.find(p => p.tipoCaminhaoId === form.tipoCaminhaoId);
    if (preco) {
      setForm(s => ({ ...s, valor: String(Number(preco.valor)).replace('.', ',') }));
    }
  }, [form.fornecedorId, form.tipoCaminhaoId, fornecedores]);
  ...
  async function registrar(e) {
    ...
    setSalvando(true);
    try {
      const { data } = await api.post('/lavagens', {
        ...form,
      ...
      setShowModal(false);
      setForm(vazioForm);
      setLavagens(prev => [data, ...prev]);
      carregarLavagens(); // reload resumo tb
    } catch (err) {
  ...
  async function excluir(id) {
    if (!confirm('Excluir este registro de lavagem?')) return;
    try {
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
        <div>
          <h2 style={{ fontSize:20, fontWeight:600, color:'#1a1a2e', margin:0 }}>Controle de Lavagens</h2>
        ...
        <button onClick={() => { setForm(vazioForm); setShowModal(true); }}
          ...
          <i className="ti ti-plus" style={{ fontSize:16 }}></i> Registrar Lavagem
        </button>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:20 }}>
        {[
          { label:'Lavagens no Mês', valor:qtdMes,    fmt: v => v,        cor:'#1a1a2e', bg:'#f8fafc', icone:'ti-wash' },
          { label:'Total Gasto',     valor:totalMes,  fmt,                 cor:'#EB3238', bg:'#fff5f5', icone:'ti-currency-dollar' },
          { label:'BUZIN',           valor:totalBuzin,fmt,                 cor:'#7c3aed', bg:'#f5f3ff', icone:'ti-truck' },
          { label:'LBM',             valor:totalLbm,  fmt,                 cor:'#b45309', bg:'#fffbeb', icone:'ti-truck' },
        ].map(c => (
          <div key={c.label} style={{ background:c.bg, border:`1px solid ${c.cor}22`, borderRadius:12, padding:'16px 18px' }}>
            ...
            <div style={{ fontSize:22, fontWeight:700, color:c.cor }}>{c.fmt(c.valor)}</div>
          </div>
        ))}
      </div>
            ...
            {['todos', ...FROTAS.map(f => f.val)].map(v => {
              ...
              return (
                <button key={v} onClick={() => setFiltroFrota(v)}
                  style={{ padding:'5px 14px', borderRadius:20, border:`1px solid ${filtroFrota===v ? (fr?.cor||'#374151') : '#d1d5db'}`,
              ...
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                <thead>
                  <tr style={{ background:'#f8fafc', borderBottom:'1px solid #e5e7eb' }}>
                    {['Data','Placa','Frota','Tipo','Fornecedor','Valor',''].map(h => (
                  ...
                  {lista.map((l, i) => {
                    ...
                    return (
                      <tr key={l.id} style={{ borderBottom:'1px solid #f3f4f6', background: i%2===0 ? '#fff' : '#fafafa' }}>
                        ...
                        <td style={{ padding:'10px 14px', color:'#374151' }}>{l.tipoCaminhao?.nome}</td>
          ...
          {loading ? (
            ...
            <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:12, overflow:'hidden' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                <thead>
                  <tr style={{ background:'#f8fafc', borderBottom:'1px solid #e5e7eb' }}>
                    {['Placa','Frota','Qtd. Lavagens','Total Gasto'].map(h => (
                      <th key={h} style={{ padding:'10px 14px', textAlign:'left', fontSize:11, fontWeight:600, color:'#6b7280', textTransform:'uppercase', letterSpacing:'0.4px' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                      ...
                      <tr key={r.placa} style={{ borderBottom:'1px solid #f3f4f6', background: i%2===0 ? '#fff' : '#fafafa' }}>
                        ...
                        <td style={{ padding:'10px 14px' }}>
                          <span style={{ display:'inline-flex', alignItems:'center', gap:6, fontWeight:600, color:'#374151' }}>
                            <span style={{ padding:'2px 10px', background:'#f0f9ff', color:'#0891b2', borderRadius:20, fontSize:13, fontWeight:700 }}>{r.quantidade}×</span>
                          </span>
                        </td>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          <div style={{ background:'#fff', borderRadius:16, width:'100%', maxWidth:480, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 12px 40px rgba(0,0,0,0.2)' }}>
            <div style={{ padding:'22px 28px 0', display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
              <h3 style={{ fontSize:16, fontWeight:700, margin:0, color:'#1a1a2e' }}>Registrar Lavagem</h3>
            ...
            <form onSubmit={registrar} style={{ padding:'0 28px 28px' }}>
              <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                    ...
                    {FROTAS.map(fr => (
                      <button key={fr.val} type="button" onClick={() => setForm(s=>({...s,frota:fr.val}))}
                        style={{ padding:'10px', border:`2px solid ${form.frota===fr.val ? fr.cor : '#e5e7eb'}`,
                          ...
                          color: form.frota===fr.val ? fr.cor : '#6b7280', fontWeight: form.frota===fr.val ? 700 : 400, fontSize:13 }}>
                ...
                <div>
                  ...
                  <select value={form.fornecedorId} onChange={e => setForm(s=>({...s,fornecedorId:e.target.value,valor:''}))} style={inp} required>
                ...
                <div>
                  ...
                  <select value={form.tipoCaminhaoId} onChange={e => setForm(s=>({...s,tipoCaminhaoId:e.target.value}))} style={inp} required>
                    <option value="">Selecione...</option>
                ...
                <div>
                  <label style={lbl}>
                    Valor (R$) *
                    {form.fornecedorId && form.tipoCaminhaoId && (
